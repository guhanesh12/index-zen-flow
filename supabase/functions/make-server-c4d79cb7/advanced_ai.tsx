...
export class AdvancedAI {
  ...
  public static async analyzeMarket(
    ...
    try {
      ...
      if (
        indexSymbol.toUpperCase() === "NIFTY" ||
        indexSymbol.toUpperCase() === "NIFTY 50" ||
        indexSymbol.toUpperCase() === "NIFTY50"
      ) {
        securityId = "13"; // NIFTY 50 security ID
        console.log(`📌 Converting NIFTY to security ID: ${securityId}`);
      } else if (
        indexSymbol.toUpperCase() === "BANKNIFTY" ||
        indexSymbol.toUpperCase() === "BANK NIFTY" ||
        indexSymbol.toUpperCase() === "BANKNIFTY50"
      ) {
  ...
  public static generateAdvancedSignal(ohlcData: OHLCCandle[], accountBalance: number = 100000): AdvancedSignal {
    ...
    console.log(
      `🔍 BODYSIZE DEBUG: bodySize=${bodySize.toFixed(2)}, close=${lastCandle.close}, open=${lastCandle.open}, bodyPercent=${bodyPercent.toFixed(1)}%, volumeRatio=${volumeRatio.toFixed(2)}`,
    );
    ...
    if (strongBullish && marketRegime.suitable_for_trading) {
      ...
      // ⚡ FIX: Use minimumBodySize (10) and hasAcceptableVolume (1.2x) for consistency

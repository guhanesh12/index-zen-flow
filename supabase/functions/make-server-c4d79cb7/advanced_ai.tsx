...
export class AdvancedAI {
  ...
  public static async analyzeMarket(
    ...
    try {
      ...
      console.log(
        `✅ [AdvancedAI] Received ${ohlcData.length} candles. Latest close: ${ohlcData[ohlcData.length - 1].close}`,
      );
  ...
  public static generateAdvancedSignal(ohlcData: OHLCCandle[], accountBalance: number = 100000): AdvancedSignal {
    ...
    const volumeRatio = avgVolume > 0 && lastCandle.volume > 0 ? lastCandle.volume / avgVolume : 0;

    // ⚡ DEBUG: Log volume data to identify issues
    console.log(
      `📊 VOLUME DEBUG: lastVolume=${lastCandle.volume}, avgVolume=${avgVolume.toFixed(2)}, volumeRatio=${volumeRatio.toFixed(2)}`,
    );
    ...
    console.log(
      `🔍 BODYSIZE DEBUG: bodySize=${bodySize.toFixed(2)}, close=${lastCandle.close}, open=${lastCandle.open}, bodyPercent=${bodyPercent.toFixed(1)}%, volumeRatio=${volumeRatio.toFixed(2)}, hasVolumeData=${hasVolumeData}`,
    );
    ...
    if (confirmationBullish && priceAboveVWAP && vwapNormalized.interpretation === "ACCEPTABLE") {
      ...
      confirmationDetails.push(
        `✅ VWAP: Price above VWAP (${vwapNormalized.distanceATR.toFixed(2)} ATR, ${vwapNormalized.distancePercent.toFixed(2)}%)`,
      );
    } else if (confirmationBearish && !priceAboveVWAP && vwapNormalized.interpretation === "ACCEPTABLE") {
      confirmations.vwap = true;
      totalWeightedScore += 2; // Weight: 2
      confirmationDetails.push(
        `✅ VWAP: Price below VWAP (${vwapNormalized.distanceATR.toFixed(2)} ATR, ${vwapNormalized.distancePercent.toFixed(2)}%)`,
      );
    ...
    if (hasVolumeData) {
      ...
      if ((isBullish || isBearish) && isVeryStrongCandle) {
        ...
        confirmationDetails.push(
          `❌ Candle Strength: Weak (${candleStrength.toFixed(1)}% body) - Vol data N/A for indices`,
        );
      }
    }
    ...
    const failedBodyCheck = hasVolumeData
      ? bodySize < minimumBodySize && bodyPercent < minimumCandleStrength
      : candleStrength < minimumCandleStrength;
    ...
    if (hasVolumeData) {
      ...
      console.log(
        `📊 VOLUME MODE: volumeRatio=${volumeRatio.toFixed(2)} (min=${minimumVolumeRatio}), bodySize=${bodySize.toFixed(2)} (min=${minimumBodySize}), bodyPercent=${bodyPercent.toFixed(1)}%, issues=${qualityIssues.join(" | ") || "none"}`,
      );
    } else {
      hasAcceptableQuality = !failedBodyCheck;
      console.log(
        `📊 INDEX MODE: candleStrength=${candleStrength.toFixed(1)}% (min=${minimumCandleStrength}%), bodySize=${bodySize.toFixed(2)} points, issues=${qualityIssues.join(" | ") || "none"}, hasVolumeData=false`,
      );
    }
    ...
    if (strongBullish && marketRegime.suitable_for_trading) {
      ...
      // ⚡ FIX: Use hasAcceptableQuality which handles both stocks (volume+body) and indices (candle strength)
    ...
    if (action !== "WAIT") {
      ...
      if (bias === "Bearish") {
        ...
        if (shouldBlock) {
          ...
          if (vwapNormalized.interpretation === "EXTENDED")
            blockReason.push(`VWAP extended (${vwapNormalized.distanceATR.toFixed(2)} ATR - reversal risk)`);
    ...
    if (action === "WAIT") {
      console.log(
        `⚠️ WARNING: Risk management values shown during WAIT action. Frontend should ignore these values when action='WAIT'.`,
      );
    }

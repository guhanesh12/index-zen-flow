...
app.post("/make-server-c4d79cb7/symbols/save", async (c) => {
  try {
    ...
    console.log(`💾 Saving ${symbols?.length || 0} trading symbols for user ${user.id}`);

    // Save symbols to KV store
    ...
    return c.json({
      success: true,
      message: `Saved ${symbols?.length || 0} symbols`,
...
app.get("/make-server-c4d79cb7/symbols/get", async (c) => {
  try {
    ...
    const data = await kv.get(`trading_symbols:${user.id}`);
    let symbols = data?.symbols || [];

    console.log(`📥 Retrieved ${symbols.length} trading symbols for user ${user.id}`);

    // ⚡⚡⚡ AUTO-CORRECTION: Fix invalid symbols automatically
    let corrected = false;
    symbols = symbols.map((symbol: any) => {
      ...
      if (symbol.exchangeSegment === 'NSE') {
        ...
        console.log(`🔧 AUTO-FIX: NSE → ${fixed.exchangeSegment} for ${symbol.displayName || symbol.name} (Index: ${symbol.index})`);
        corrected = true;
    ...
    // ✅ If any corrections made, save back to database
    if (corrected) {
      console.log(`💾 Auto-saving ${symbols.length} corrected symbols to database...`);
      await kv.set(`trading_symbols:${user.id}`, {
        symbols,
        lastUpdated: Date.now()
      });
    ...
    console.error('❌ Error retrieving trading symbols:', error);
    return c.json({ error: error.message }, 500);
  }
});

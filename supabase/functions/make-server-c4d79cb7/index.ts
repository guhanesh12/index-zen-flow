...
// ⚡ Backtest a fixed date range across multiple indices (no DB writes)
app.post("/make-server-c4d79cb7/backtest/dates", async (c) => {
  try {
    const body = await c.req.json();
    const {
      userId,
      ...
      capital = 100000,
      optionDelta = 0.5,              // ATM proxy for option P&L
    } = body;
    if (!userId || !fromDate || !toDate) {
      return c.json({ success: false, error: "userId, fromDate, toDate required" }, 400);
    }
    const credentials = (await kv.get(`api_credentials:${userId}`)) as any;
    const token = credentials?.dhanAccessToken;
    if (!token) return c.json({ success: false, error: "Dhan credentials not configured for user" }, 400);

    const securityIdMap: Record<string, string> = { NIFTY: "13", BANKNIFTY: "25", SENSEX: "51" };
    const lotSizeMap: Record<string, number> = { NIFTY: 75, BANKNIFTY: 30, SENSEX: 20 };
    const segmentMap: Record<string, string> = { NIFTY: "IDX_I", BANKNIFTY: "IDX_I", SENSEX: "IDX_I" };

    const results: any[] = [];
    for (const idx of indices) {
      const reqBody = {
        securityId: securityIdMap[idx],
        exchangeSegment: segmentMap[idx],
        instrument: "INDEX",
        interval: parseInt(interval),
        oi: false,
      ...
      if (!r.ok || !d.open || !d.close) {
        results.push({ index: idx, error: "dhan_fetch_failed", status: r.status, raw: d });
        continue;
      }
      const { open, high, low, close, volume, timestamp } = d;
      const { signals, summary, candles } = runManualStrategy(open, high, low, close, volume, timestamp);
      const lotSize = lotSizeMap[idx];

      // Capital-sized: use 1 lot for futures-equivalent P&L per index
      const { trades, stats } = simulateTrades(signals, lotSize, candles);

      // Filter signals/trades to within requested window only (strict date)
      const inWindow = (ts: number) => {
        const ms = ts < 1e12 ? ts * 1000 : ts;
        const d2 = new Date(ms).toISOString().slice(0, 10);
        return d2 >= fromDate && d2 <= toDate;
      };
      const tradesInWindow = trades.filter((t: any) => {
        // entryTime is locale string; use signals[].timestamp via match on entryPrice+entry — simpler: re-derive from signal list
        return true; // candle fetch already constrained to date range
      });
      const signalsAll = signals.filter((s: any) => s.action !== "WAIT" && inWindow(s.timestamp));

      // Option P&L estimate: points * delta * lotSize per trade
      const optionTrades = tradesInWindow.map((t: any) => {
        const optionPnl = t.pointsMoved * optionDelta * lotSize;
        return { ...t, optionPnl: Number(optionPnl.toFixed(2)) };
      });
      const optionPnlSum = optionTrades.reduce((s: number, t: any) => s + t.optionPnl, 0);

      // Capital-based lots for NIFTY-style option (assume avg ATM premium ~ price*0.005 for index)
      // Per lot premium estimate: lotSize * estPremium where estPremium ~ 150 (typical ATM)
      const avgPremium = idx === "NIFTY" ? 150 : idx === "BANKNIFTY" ? 250 : 300;
      const perLotCost = avgPremium * lotSize;
      const lotsAffordable = Math.max(1, Math.floor(capital / perLotCost));
      const scaledOptionPnl = optionPnlSum * lotsAffordable;
      ...
      results.push({
        ...
        dateRange: {
          from: fromDate,
          to: toDate,
          firstCandle: candles[0] ? new Date(candles[0].timestamp * 1000).toISOString() : null,
          lastCandle: candles[candles.length - 1]
            ? new Date(candles[candles.length - 1].timestamp * 1000).toISOString()
            : null,
          totalCandles: candles.length,
        },
        signalSummary: summary,
        signalCount: signalsAll.length,
        signals: signalsAll.map((s: any) => ({
          time: s.date, action: s.action, price: s.price, confidence: s.confidence,
          sl: s.stopLoss, tp: s.target, reasoning: s.reasoning,
        })),
        tradeStats: stats,
        futuresPnL_1Lot: stats.totalPnL,
        optionTrades,
        optionAnalysis: {
          assumedDelta: optionDelta,
          assumedAvgPremium: avgPremium,
          perLotPremiumCost: perLotCost,
          capital,
          lotsAffordable,
          optionPnL_1Lot: Number(optionPnlSum.toFixed(2)),
          optionPnL_Scaled: Number(scaledOptionPnl.toFixed(2)),
          roi_pct: Number(((scaledOptionPnl / capital) * 100).toFixed(2)),
        },
      });
    }

    return c.json({ success: true, fromDate, toDate, interval, capital, results });
  } catch (e: any) {
    console.error("❌ /backtest/dates error", e);
    return c.json({ success: false, error: e.message, stack: e.stack?.substring(0, 400) }, 500);
  }
});
...
// 🔁 Restart stuck VPS provisioning using the current DigitalOcean token without charging again
app.post("/make-server-c4d79cb7/ip-pool/provisioning-restart", async (c) => {
  try {
    const { user, error } = await validateAuth(c);
    if (error || !user) {
      return c.json({ code: error.code, message: error.message }, error.code);
    ...
    if (assignment?.ipAddress && assignment.subscriptionStatus === 'active') {
      return c.json({
        success: false,
        error: 'Active VPS already assigned. Restart is only for stuck provisioning.'
      }, 400);
    }

    const existingJob = await VPSProvisioning.getUserProvisioningJob(user.id);
    if (!existingJob) {
      return c.json({ success: false, error: 'No stuck provisioning job found to restart.' }, 400);
    }

    const cancelResult = await VPSProvisioning.cancelUserProvisioningJob(user.id);
    if (!cancelResult.success) {
      return c.json({ success: false, error: cancelResult.error || 'Failed to cancel old provisioning' }, 500);
    }
    ...
    return c.json({
      ...
      deletionError: cancelResult.deletionError
    });
  } catch (error: any) {
    console.error('❌ Restart provisioning error:', error);
    return c.json({ error: error.message }, 500);
  }
});

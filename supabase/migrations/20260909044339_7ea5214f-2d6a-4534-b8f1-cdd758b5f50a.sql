...
BEGIN
  IF NEW.is_active = false AND (OLD.is_active IS DISTINCT FROM NEW.is_active) THEN
    ...
    -- Engine must switch OFF for ALL real position closures (auto exits, broker/external
    -- closes and manual user exits). Only bookkeeping rows are ignored.
    IF reason NOT LIKE '%duplicate%' AND reason NOT LIKE '%housekeep%'
       AND reason NOT LIKE '%cleanup%' AND reason NOT LIKE '%stale%'
       AND reason NOT LIKE '%never opened%' THEN
      BEGIN
